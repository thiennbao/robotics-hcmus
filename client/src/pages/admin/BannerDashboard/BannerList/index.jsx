...
const BannerList = () => {
  ...
  const banners = rawBanners.map((rawBanner) => ({
    date: rawBanner.createdAt.split("T")[0],
    ...rawBanner,
  }));
  ...
  const handleImport = (e) => {
    ...
    readFile(file, (res) => {
      ...
      data.forEach((item) => {
        resourceApi
          .postResource({ resource: "banner", data: item })
          .then(() => dispatch(getBanners({ skip: 0, limit: 5 })))
          .catch((error) => console.log(error));
      });
    });
  };
    ...
    <AdminLayout page="BANNER">
      ...
      <div className="d-flex justify-content-center my-5">
        ...
        <Button variant="outline" color="green" className="mx-2">
          <a href={downloadUrl} download="banner.json" className="px-4 py-2" style={{ color: "inherit" }}>
            Download data
          </a>
        </Button>
